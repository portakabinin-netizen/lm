...
const resolveCorpAdmin = async (req) => {
  ...
  if (!admin) {
    console.warn(
      `⚠️ [resolveCorpAdmin] No CorpAdmin found for dbName: ${tDb}. User: ${req.user?.userId} [${req.user?.userRole}]`
    );
  }
...
const updateCorporate = {
  ...
  getCorporate: async (req, res) => {
    try {
      ...
      if (!ProfileMaster)
        return res.status(500).json({ message: 'Tenant connection not resolved' });
    ...
    try {
      ...
      if (!validation.isValidPAN(pan)) {
        return res.status(400).json({
          message: 'A valid 10-character Corporate PAN is required for database isolation.',
        });
      }
      ...
      if (existingOwner) {
        return res.status(400).json({
          message: `Corporate with PAN ${pan} is already registered to ${existingOwner._id === req.user.userId ? 'your' : 'an'} account.`,
        });
      }
      ...
      if (user.accessCorporate.some((c) => c.dbName === dbName)) {
        return res
          .status(400)
          .json({ message: 'This corporate is already linked to your account' });
      }
  ...
  postAdminUser: async (req, res) => {
    try {
      ...
      const updated = await userMaster
        .findByIdAndUpdate(req.user.userId, { $set }, { new: true, runValidators: true })
        .lean();
      ...
      if (err.code === 11000) {
        return res
          .status(409)
          .json({ message: 'Duplicate record: Aadhar or Mobile already exists' });
      }
      return res
        .status(500)
        .json({ message: 'Server error during profile update', error: err.message });
    }
  },
};
...
const otherUser = {
  ...
  getOtherUserById: async (req, res) => {
    try {
      ...
      const user = await userMaster.findOne(query, '-userPassword').lean();
  ...
  searchUser: async (req, res) => {
    try {
      ...
      const users = await userMaster
        .find(
          {
            $or: [{ userMobile: query.trim() }, { userAadhar: query.trim() }],
          },
          'userDisplayName userEmail userMobile userAadhar userRole userActive accessCorporate'
        )
        .lean();
  ...
  assignCorporate: async (req, res) => {
    try {
      ...
      if (Array.isArray(corporateIds)) {
        ...
        const selectedLinks = corporateIds
          .map((corpId) => {
            ...
            return {
              ...
              locationId: req.body.locationId
                ? new mongoose.Types.ObjectId(req.body.locationId)
                : undefined,
              isActive: true,
            };
          })
  ...
  unassignCorporate: async (req, res) => {
    try {
      ...
      targetUser.accessCorporate = (targetUser.accessCorporate || []).filter(
        (c) => c.dbName !== targetDb
      );
      await targetUser.save();
